			...
			onTheMasterPage : {
				...
				actions : {
										...
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("Material") === oObject.Material;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].Material;
										} else {
											sCurrentId = aItemsNotInTheList[0].Material;
										}
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							title: oBindingContext.getProperty("Material")
						};
					}
				},
				...
				assertions : {
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("Material");
					},
